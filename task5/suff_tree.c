...
int32_t st_suflink(const char* str, st_tree* tree, st_node* node)
{
    ...
    st_state tr = st_transition(str, tree, state, left, right);
    node->m_suflink = st_split(str, tree, tr);
    return node->m_suflink;
}

// extend suffix tree corresponded to str from [0 ... pos] to [0 ... pos+1]
st_state st_extend_tree(const char* str, int32_t len, st_tree* tree, st_state state, int32_t pos)
{
    while(true) {
...
st_tree* st_create_tree(const char* str, int32_t len)
{
    ...
    tree->m_nodes = (st_node*) malloc(2 * len * sizeof(st_node));
    tree->m_size  = 1;
    // init root
    ...
    for (int32_t i = 0; i < len; ++i) {
        state = st_extend_tree(str, len, tree, state, i);
    }
